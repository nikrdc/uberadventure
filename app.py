...
yelp_api = YelpAPI(config.YELP_CONSUMER_KEY, config.YELP_CONSUMER_SECRET, config.YELP_TOKEN, 
                   config.YELP_TOKEN_SECRET)
...
def index():
    ...
    if form.validate_on_submit():
        ...
        town = str(nomrev.query(lat=start_lat, lon=start_lon)['address']['city'])
