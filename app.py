...
def index():
    ...
    if form.validate_on_submit():
        ...
        for i in xrange(len(uber_products)):
            if uber_products[i]['display_name'] == 'uberX':
                uber_index = i
                break
        ...
        travel_money = amount - uber_price_details['base'] - uber_service_fees
        print("determined remaining travel cash")

        radius_meters = travel_money / (uber_price_details['cost_per_distance']
        ...
        print("found " + town)
        yelp_data = yelp_api.search_query(
            location=town, 
            sort=2,
        ...
        while high_estimate > amount:
            ...
            uberX_price = uber_price_estimate[uber_index]
            product_id = uberX_price['product_id']
            high_estimate = uberX_price['high_estimate']
        ...
        deep_link = "uber://?client_id=" + config.UBER_CLIENT_ID +\
                    ...
                    str(chosen_lat) + "&dropoff[longitude]=" + str(chosen_long) +\
        ...
        return render_template('boom.html', 
                               deep_link=deep_link, 
                               d_name=chosen['name'], 
                               d_address=display_address,
                               d_cost=uberX_price['estimate'],
                               d_rating=chosen['rating'],
                               d_url=chosen['mobile_url'],
                               d_product=uberX_price['display_name'],)
    else:
